...
from pytest import Collector, Item, Config, Session, Parser
...
def pytest_sessionstart(session: Session) -> None :
    ...
    session.infrahub_repo_config = load_repository_config(repo_config_file=config_path)

def pytest_collect_file(parent: Union[Collector, Item], file_path: Path):
...
def pytest_configure(config: Config) -> None:
    ...
    config.addinivalue_line("markers", "infrahub_transform_python(name: str): Test related to a Transform Python for Infrahub")
