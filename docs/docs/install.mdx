...
:::info
uv automatically manages virtual environments and uses the `uv.lock` file to ensure reproducible builds.
:::
...
- Try removing the `uv.lock` file and running `uv sync` again
- Check which Python version uv is using: `uv python list`
