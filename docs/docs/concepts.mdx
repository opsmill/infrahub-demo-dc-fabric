...
The schema files in `models/` define these object types, their attributes, and how they relate to each other. For example, a device has interfaces, belongs to a location, runs a specific platform, and participates in a topology.
...
- Easy scaling by modifying quantities in topology elements
...
- Easy modifications to topology scale
