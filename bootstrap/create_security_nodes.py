...
RULES = [
    SecurityPolicyRule(
        name="permit-inbound-smtp",
        index=0,
        policy=GLOBAL_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_OUTSIDE,
        destination_zone=ZONE_DMZ,
        source_addresses=[ANY],
        destination_groups=[SMTP_SERVERS],
        destination_services=[SMTP],
    ),
    SecurityPolicyRule(
        name="deny-smtp-servers-outbound",
        index=1,
        policy=GLOBAL_POLICY,
        action=PolicyAction.deny,
        source_zone=ZONE_DMZ,
        destination_zone=ZONE_OUTSIDE,
        source_groups=[SMTP_SERVERS],
        destination_addresses=[ANY],
        destination_services=[IP],
    ),
    SecurityPolicyRule(
        name="permit-web-proxies-outbound",
        index=0,
        policy=EUROPE_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_DMZ,
        destination_zone=ZONE_OUTSIDE,
        source_groups=[EUR_WEB_PROXIES],
        destination_addresses=[ANY],
        destination_service_groups=[HTTP_HTTPS],
    ),
    SecurityPolicyRule(
        name="deny-web-proxies-ip-outbound",
        index=1,
        policy=EUROPE_POLICY,
        action=PolicyAction.deny,
        source_zone=ZONE_DMZ,
    ...
    SecurityPolicyRule(
        name="permit-internal-to-webproxies",
        index=2,
        policy=EUROPE_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_INSIDE,
        destination_zone=ZONE_DMZ,
        source_addresses=[ANY],
        destination_groups=[EUR_WEB_PROXIES],
        destination_services=[HTTP_PROXY],
    ),
    SecurityPolicyRule(
        name="permit-smpt-servers-icmp",
        index=3,
        policy=EUROPE_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_DMZ,
    ...
    SecurityPolicyRule(
        name="deny-block-internet-internal",
        index=0,
        policy=FRA_POLICY,
        action=PolicyAction.deny,
        source_zone=ZONE_OUTSIDE,
        destination_zone=ZONE_INSIDE,
        source_groups=[BLOCK_INTERNET],
        destination_addresses=[ANY],
        destination_services=[IP],
    ),
    SecurityPolicyRule(
        name="deny-block-internet-dmz",
        index=1,
        policy=FRA_POLICY,
        action=PolicyAction.deny,
        source_zone=ZONE_OUTSIDE,
    ...
    SecurityPolicyRule(
        name="permit-internal-ssh-smtp-servers",
        index=2,
        policy=FRA_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_INSIDE,
        ...
        source_addresses=[ANY],
        destination_groups=[SMTP_SERVERS],
        destination_services=[SSH],
    ),
    SecurityPolicyRule(
        name="permit-extranet-ssh-internal",
        index=999,
        policy=FRA_POLICY,
        action=PolicyAction.permit,
        source_zone=ZONE_EXTRANET,
        destination_zone=ZONE_INSIDE,
        source_addresses=[ANY],
        destination_addresses=[ANY],
        destination_services=[SSH],
    ...
    SecurityPolicyRule(
        name="deny-internal-block-internet",
        index=0,
        policy=FRA_FW1_POLICY,
        action=PolicyAction.deny,
        source_zone=ZONE_INSIDE,
        destination_zone=ZONE_OUTSIDE,
        source_addresses=[ANY],
        destination_groups=[BLOCK_INTERNET],
        destination_services=[IP],
    ),
]
...
async def run(client: InfrahubClient, log: logging.Logger, branch: str) -> None:
    for ip_proto in IP_PROTOCOLS:
        obj = await client.create(
            ...
            name=ip_proto.name,
            protocol=ip_proto.protocol,
            description=ip_proto.description,
        )
    ...
    for rule in RULES:
        ...
        source_service_groups = (
            ...
            if rule.source_service_groups
            else None
        )
        destination_address = (
            [store_get_or_none(address.name) for address in rule.destination_addresses]
        ...
        destination_service_groups = (
            [store_get_or_none(group.name) for group in rule.destination_service_groups]
            if rule.destination_service_groups
            else None
        )
        obj = await client.create(
            ...
            source_service_groups=source_service_groups,
            destination_address=destination_address,
            destination_groups=destination_groups,
            destination_services=destination_services,
            destination_service_groups=destination_service_groups,
        )
    ...
    device_type = await client.create(
    ...
    interfaces = [
        ("ge-0/0/1", "outside", "10.0.1.1/24"),
        ("ge-0/0/2", "inside", "10.0.2.1/24"),
        ("ge-0/0/3", "dmz", "10.0.3.1/24"),
    ]

    for interface, zone, ip in interfaces:
        security_zone = store_get_or_none(zone)

        firewall_interface = await client.create(
            kind="SecurityFirewallInterface",
            name=interface,
            speed=1_000_000,
