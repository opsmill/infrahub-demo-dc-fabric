...
class NetworkTopologyGenerator(InfrahubGenerator):
    async def generate(self, data: dict) -> None:
        ...
        try:
            ...
            populate_local_store(objects=accounts, key_type="name", store=self.client.store)

            tenants = await self.client.all("OrganizationTenant")
            populate_local_store(objects=tenants, key_type="name", store=self.client.store)

            providers = await self.client.all("OrganizationProvider")
            populate_local_store(objects=providers, key_type="name", store=self.client.store)

            manufacturers = await self.client.all("OrganizationManufacturer")
            populate_local_store(objects=manufacturers, key_type="name", store=self.client.store)

            autonomous_systems = await self.client.all("InfraAutonomousSystem")
            populate_local_store(
                objects=autonomous_systems, key_type="name", store=self.client.store
            )

            platforms = await self.client.all("InfraPlatform")
            populate_local_store(objects=platforms, key_type="name", store=self.client.store)

            device_types = await self.client.all("InfraDeviceType")
            populate_local_store(objects=device_types, key_type="name", store=self.client.store)

            topologies = await self.client.all("TopologyTopology")
            populate_local_store(objects=topologies, key_type="name", store=self.client.store)

            evpn_strategies = await self.client.all("TopologyEVPNStrategy", populate_store=True)
            populate_local_store(
                objects=evpn_strategies, key_type="name", store=self.client.store
            )

            locations = await self.client.all("LocationGeneric", populate_store=True)
            populate_local_store(objects=locations, key_type="name", store=self.client.store)

            groups = await self.client.all("CoreStandardGroup")
            populate_local_store(objects=groups, key_type="name", store=self.client.store)
            
            prefixes = await self.client.all("InfraPrefix")
            populate_local_store(objects=prefixes, key_type="prefix", store=self.client.store)

            vrfs = await self.client.all("InfraVRF")
            populate_local_store(objects=vrfs, key_type="name", store=self.client.store)
