...
async def generate_network_services(
    ...
    if service_type.title() == "Layer3":
        if not location_supernet:
            log.error(
                f"{topology.location.peer.name.value} doesn't have any supernet, we are not able to define which prefixes we can used."
            )
            return None
        # FIXME
        # Replace Section when we have Ressource Manager
            ...
            for locations_subnet in locations_subnets:
                if locations_subnet.role.value in ("server"):
                    existing_prefixes += 1
            all_prefixes = list(
                IPv4Network(location_supernet.prefix.value).subnets(new_prefix=24)
            )
            remaining_prefixes = all_prefixes[existing_prefixes:]
            if len(remaining_prefixes) < 4:
                log.error(
                    f"The number of prefixes still available in {location_supernet.prefix.value} doesn't allow to create the requested services"
                )
                return None
    ...
    existing_vlans = [
        location_vlan
        for location_vlan in locations_vlans
        if location_vlan.role.value == "server"
        and location_vlan.vlan_id.value // 100 == vlan_prefix_to_match
    ]
    service_identifiers = await client.filters(
        kind="TopologyNetworkServiceIdentifier", branch=branch, populate_store=True
    )
    identifier_prefix_to_match = int(f"{topology_index}{vrf_index}")
    existing_services = [
        identifier
        for identifier in service_identifiers
        if int(identifier.identifier.value) // 100 == identifier_prefix_to_match
    ]
    ...
    new_service_id = (
        service_id
        if service_id
        else identifier_prefix_to_match * 100 + len(existing_services) + 1
    )
    params = {
    ...
    vlan_id = (
        previous_vlan_id
        if previous_vlan_id > 0
        else vlan_prefix_to_match * 100 + len(existing_vlans) + 1
    )
    ...
    service_name = (
        ...
        if previous_service_name and previous_service_name.startswith(service_prefix)
        else service_prefix + "_server_" + str(new_service_id)
    )
    service_description = (
        f"{service_prefix.upper()} Service in {vrf_name} VRF on {topology_name}"
    )
    ...
    async with client.start_tracking(params=params, delete_unused_nodes=True) as client:
        ...
        vlan_data = {
            ...
            "description": {
                "value": f"{location_name.upper()} - {vlan_name.lower()} VLAN"
            },
        ...
        if service_type.title() == "Layer3":
            prefix_prefix = (
                previous_prefix if previous_prefix else remaining_prefixes[1]
            )
...
async def run(
    ...
    if not service_id:
        log.info(
            f"Generation of a {service_type.title()} services in {vrf_name.title()} VRF on {topology_name}"
        )
    else:
        log.info(
            f"Creating or updating {service_id} based on the others parameters given"
        )
    ...
    if not service_id:
        log.info(
            f"Created new {service_type.title()} services in {vrf_name.title()} VRF on {topology_name}"
        )
    else:
